...
public class OAuthManagerBuilder {
    ...
    /**
     * Build oauth manager based on the current configuration.
     *
    ...
    public OAuthManagerBuilder register(OAuthManagerProperties properties) {
        properties.check();
        OAuthRequestExecutorProperties defaultRequestExecutorProperties = properties.getDefaultRequestExecutor();
        properties.getClients().forEach(client -> {
            if (client.getRequestExecutor() == null) {
                client.setRequestExecutor(new OAuthRequestExecutorProperties());
            }
            client.getRequestExecutor().fillNullPropertiesWith(defaultRequestExecutorProperties);
            String expr = client.getOpenPlatformExpr();
            OAuthAppProperties app = client.getApp();
            OAuthRequestExecutor executor = newRequestExecutor(client.getRequestExecutor());
     ...
     * @see com.github.wautsns.okauth.core.client.kernel.model.properties.OAuthClientProperties#setOpenPlatformExpr(String)
     */
     ...
     * @return enum constant of the specific name, or {@code null} if no enum constant is named the specific name
     */
     ...
     */
    private static OAuthRequestExecutor newRequestExecutor(OAuthRequestExecutorProperties properties) {
        try {
