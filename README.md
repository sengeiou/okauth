...
1. [`OAuthAuthorizeUrlExtendedQuery`](/okauth-core/src/main/java/com/github/wautsns/okauth/core/client/core/dto/OAuthAuthorizeUrlExtendedQuery.java "点击查看源码")  
	该类是 OAuth2.0 授权码模式中, 生成 authorize url 时, 能够携带的额外参数, 目前包含两个属性 `state` 与 `scope` .
...
4. [`OAuthToken`](/okauth-core/src/main/java/com/github/wautsns/okauth/core/client/core/dto/OAuthToken.java "点击查看源码")  
	...
	3. `String getString(String name)` : 获取指定名称的属性值,  若不为 `String` 类型, 则会调用 `toString()`
...
5. [`OAuthUser`](/okauth-core/src/main/java/com/github/wautsns/okauth/core/client/core/dto/OAuthUser.java "点击查看源码")  
	...
	4. `T get(String name)` : 获取指定名称的属性值
	5. `String getString(String name)` : 获取指定名称的属性值,  若不为 `String` 类型, 则会调用 `toString()`
	6. `Map<String, Object> getMap(String name)` : 获取指定名称的 `Map`, 若类型不为 `Map` 则会返回 `null`
	7. `Map<String, Object> getOriginalDataMap()` : 获取原始数据集
6. [`OkAuthClient`](/okauth-core/src/main/java/com/github/wautsns/okauth/core/client/core/OkAuthClient.java "点击查看源码")  
	...
	3. `OAuthToken exchangeForToken(OAuthRedirectUriQuery query)` : 用 `query` 的属性 `code` 交换令牌
	4. `OAuthUser exchangeForUser(OAuthToken)` : 用令牌交换用户信息
	5. `OAuthUser exchangeForUser(OAuthRedirectUriQuery query)` : 用 `query` 的属性 `code` 直接交换用户信息
...
// ...
	...
	@GetMapping("/cmd/init-oauth-authorize-url")
	public String initAuthorizeUrl(String openPlatform) throws OkAuthException {
